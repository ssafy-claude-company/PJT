...
- **문제**: spine 동작이 테스트로 고정돼 있지 않은 영역(완료 게이트·정밀 복구·자동 이어가기/위임/조율)이 리팩토링 회귀에 무방비. deploy는 *전용 테스트 모듈이 없고* `test_sys`에서 대개 `deploy_sync`를 monkeypatch로 대체해, `_http`/`_git` 같은 라이브 경로의 직접 단위 테스트가 얇다(단, `_free_slots` orphan 수거 등 일부 핵심 로직은 `test_sys`에서 검증됨 — 독립 검증이 '전용 테스트 없음'을 과장으로 판정해 교정). `deploy.py:153`
...
- **문제**: 비활성 게이트 ~100줄(`permissions.py:310`), 프로덕션 허용목록과 다이버전스한 `organt_allowed_tools`(`:9`, 테스트만 사용 — '데드' 아님), 오해를 주는 `build_options` 기본값(`organt.py:109`), 고아 `channels.py`(`:40`).
