...
| R-B2-2 | `organt_allowed_tools` 데드·다이버전트 | `permissions.py:9` | read |
...
| R-B8-1 | Kind 파싱 불일치 + silent INFO | `guide_tools.py:809` ↔ `protocol.py:126` ↔ `permissions.py:40` | spot |
...
| R-S7-1 | 공유 헬퍼 집 없음(함수-로컬 import) | `permissions.py:294`, `sys_core.py:593` | read |
| R-S8-1 | 스테일 문서 참조(존재하지 않는 docs/ 파일) | `communication.py:3`, `protocol.py:1`, `discord_guide.py:4` | spot/wf |

## E. 의존성 (02)
| ID | 주장 | file:line | 등급 |
|----|------|-----------|:----:|
| R-DEP-1 | 공유 헬퍼 역방향 결합 | `permissions.py:294`, `sys_core.py:593` | read |
...
3. **병렬 분석(wf)**: 13개 모듈을 독립 에이전트가 분석 → 본 색인의 `discord_guide`·`deploy`·`channels` 등 비-spine 모듈 근거 제공. 이 발견들은 적대적 검증(인용 정확성 + 주장 방어가능성) 대상이며, 최종 검증 집계는 PR 본문에 첨부한다.
4. **교차 정합**: 병렬 에이전트가 본 문서 초안을 교차검토해 자기참조 정합을 확인(예: 03 문서의 enforcement 라인 인용 일치 확인).

> 등급이 `wf`인 항목 중 보안·정확성 핵심(R-B5·R-B6)은 [07](../07-refactoring-targets.md) P0에서 우선 재확인·교정 대상으로 둔다.
