...
- `organt_allowed_tools`(`permissions.py:9`) — **데드는 아님**(테스트가 사용: `tests/test_permissions.py:9`, `tests/test_sys.py:301`). 다만 프로덕션 허용목록(`main.py`의 `WORKER_BASE_TOOLS`+`FLOW_TOOLS`)과 **다이버전스**해, 테스트가 프로덕션이 안 쓰는 목록으로 훅을 검증한다(일관성 갭). `[R-B2-2]` ※독립 검증이 '데드' 표현을 반증 → 교정함
...
### B8 · 일관성 결여 — Kind 파싱 3종 〔low〕
같은 'Work 판정'이 세 곳에서 다르게 구현된다: `startswith("w")`(`guide_tools.py:809`) vs `startswith("work")`(`protocol.py:126`) vs 소문자 동등(`permissions.py:40`). **유효 입력("Work"/"Info")은 셋 다 올바르게 매핑**되므로 실해는 낮다(독립 검증이 '오타가 의도와 다르게 흐른다'는 위험을 *과장*으로 판정 → 강등). 다만 `"w…"`류 오입력 시 `guide_tools`는 Work, `protocol`은 비-work→INFO로 갈려 *일관성/유지보수* 냄새는 남는다. → 단일 파서로. `[R-B8-1]`
...
### S7 · 공유 헬퍼의 집 없음(역방향 reach-in) 〔med〕
`_jobs_of`/`_norm_job`/`_speech_clip`/`_CAPS`가 `guide_tools`에 살며 `permissions`·`sys_core`·`discord_guide`가 **함수-로컬 import**로 끌어쓴다(`permissions.py:294`, `sys_core.py:593`). **순환의존은 아니다**(독립 검증 확인: `guide_tools`는 이들을 되-import하지 않음) — 정책·전송 계층이 도구 모듈 *내부*로 손을 뻗는 역방향 reach-in/계층 냄새이고, 함수-로컬 import는 그 결합을 숨긴다. → 공용 유틸 모듈로 추출. `[R-S7-1]`
...
- **독립 적대적 검증 집계**: 154 에이전트·~2.69M 토큰 규모의 다중에이전트 분석이 **140개 발견을 전부 재검증**(소스 재대조). **129개(92%) 주장 유지**, 11개는 *과장/오류*로 판정, 인용 4건 라인 보정. 이 11건 중 본 문서에 반영됐던 4건(`organt_allowed_tools` '데드'·deploy '전용 테스트 없음'·`_jobs_of` '순환의존'·Kind '오타 위험')은 **본 문서·`ref/`에 교정 반영**(위 B2·B8·S7 및 [07](07-refactoring-targets.md) P0).
- 동시 커밋이 `src/sys_core.py`만 수정했으나, 본 문서의 모든 인용 anchor(함수 def 라인)는 **현재 커밋 파일과 일치 재확인**(라인 드리프트 없음).
