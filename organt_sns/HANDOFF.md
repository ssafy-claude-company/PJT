...
- **설계 원칙** — 디스코드 제약은 풀렸다. **디스코드 구현 형태를 흉내내지 말 것**(상태를 채팅으로
  ...
  웹**으로 표현한다. (§3)
- **다음(방향 전환 2026-06-25)** — **현재 구조(Render) 유지**. 자체 서버 이전은 **보류**(나중 옵션).
  이번 초점은 **웹사이트 고도화 + E2E 테스트 + 사용자 경험(UX)** — 사용자가 직접 써보며 다듬는다. (§6, §10)
...
## 6. 다음 작업 — 웹 고도화 + E2E + UX (현재 구조 유지)

> **방향 전환(2026-06-25).** 자체 서버 이전은 **보류**. 당분간 **현재 구조(Render + 원격 러너)** 를
> 그대로 두고, 사용자가 직접 써보며 **웹사이트를 고도화**하고 **E2E 테스트**로 굳히며 **UX를 다듬는다.**
> 작업거리는 **§10 검수 백로그**에 우선순위로 있다. 이번 세션에 잔 UX 문제(날짜 표시·빈/오류 상태·
> 무피드백 액션·raw 데이터 노출)는 처리 완료 — 남은 건 **구조적 고도화**(타임라인 회의 블록·페이즈
> 레일·목록 섹셔닝)와 **E2E**, 그리고 **서버 안정성**(자체서버 production 시점).

---

### (참고·보류) 자체 서버 이전 — task #19

자체 서버로 갈 때의 목표·절차. 지금은 **보류**지만 자료는 유지한다.
...
## 10. 검수 백로그 (2026-06-25, 우선순위순)

핸드오프 전 3축(서버 안정성·디자인·사용성) 검수에서 나온 항목. **회의 표시 픽스 검증 완료**
(`sns/tests.py::MeetingVisibilityTest`).

**✅ 이번 세션 처리(머지 전 잔 UX 문제):**
- 날짜 표시 — 활동 피드/협업패널이 `시:분`만 → `whenFmt`(방금/N분 전/어제/날짜) + 날짜 구분선
  (`kinds.js whenFmt/dayLabel`, `EventItem.vue`, `AgentDetail.vue` feedRows, `CollabPanel.vue`).
- 빈/오류 상태 — Channel 로드 실패 시 빈 화면 → 재시도 안내; Studio/Agents 로드 try/finally(영구
  스피너 방지); Recommend 0건 안내; Agents 빈 카피("공개된 직원이 없어요").
- 무피드백 액션 — 토스트 도입(`toast.js`, App.vue): 보관/공개전환/이름변경/초대/친구 수락·거절·삭제/
  공유/채용/메시지 실패. doArchive는 로컬 상태도 갱신(메뉴 라벨 최신화).
- raw 데이터 — 채널 목록 raw pid 노출 제거(리더/“채팅 채널”로), CollabPanel 위임 last의 봇 id 제거,
  EventItem 로그 접두 정리. 백엔드 에러 카피 봇→직원(`views.py`).
- **활동 피드 '거부' 도배** — `tool_denied`(게이트 거부)는 SYS 안전장치가 '리더 독식/권한 밖 도구/
  타 도메인 대리구현' 등을 막은 **내부 기록**(직원이 '한 일'이 아니라 '막힌 시도'). 최근60 피드 창에
  몰리면 빨간 거부가 도배돼 보였다(라이브 리더 20/60=33%). **활동은 정상**(라이브 감사 tool_use 104 :
  denied 6 = 95% 성공, 시드 리더는 work 74%·denied 6%) — 핵심 문제 아님. → 활동 피드/채널에서
  `denied` 제외(`views.py` agent.events·project.events·messages). 데이터는 DB에 남음(향후 게이트 뷰용).
  (참고: `raw`/'기타'도 노이즈 후보 — 이번엔 거부만 surgical 제외.)

**남은 작업(새 세션 — 웹 고도화 + E2E):** 아래 A(서버, 자체서버 시점)·B(사용성 기능)·C 구조 고도화.
...
- **[목록 섹셔닝]** *(부분 완료)* — AgentDetail 활동피드 날짜그룹·raw 정리 ✅, Channels raw pid ✅.
  남은 것: Channels 내/공개 **섹션 분리** + 정렬 컨트롤, Friends 5개 평면 리스트 **그룹화**(받은 요청 vs
  내 친구). Agents.vue/Recommend.vue가 좋은 템플릿.
