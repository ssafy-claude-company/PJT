...
def _normalize_audit(rec: dict, ts: float) -> Optional[Event]:
    ...
    if ev == "user_request":
        return Event(
            ts=ts,
            source="audit",
            kind=KIND_USER_REQUEST,
            target=rec.get("to"),
            summary=_clip(rec.get("body"), 120),
    ...
    if tool == _GUIDE + "request":
        ...
        if kind_w:
            return Event(
                ...
                role=role,
                target=to,
                summary=f"{role or actor} → {to}: {body}",
                payload={"body": ti.get("body")},
            )
        return Event(
            ts=ts,
            source="audit",
    ...
    if tool in (_GUIDE + "set_goal",):
        return Event(
            ...
            actor=actor,
            role=role,
            summary=f"{role or actor}: 목표 확정 — {_clip(ti.get('goal'), 80)}",
            payload=dict(ti),
        )
    ...
    if tool == _GUIDE + "complete_task":
        return Event(
            ...
            actor=actor,
            role=role,
            summary=f"{role or actor}: Task 마감",
            payload=dict(ti),
        )
    ...
    if tool == _GUIDE + "recruit":
        return Event(
            ...
            actor=actor,
            role=role,
            summary=f"{role or actor}: 충원 — {_clip(ti.get('role'), 40)}",
            payload=dict(ti),
        )
...
def _normalize_flow(rec: dict, ts: float) -> Optional[Event]:
    ...
    m = {
        "role_distilled": (
            KIND_LEARN,
            f"{rec.get('job')} 직무기준 증류(경험 {rec.get('used')}건 압축)",
        ),
        "role_profile_saved": (
            KIND_LEARN,
            f"{rec.get('job')} 직무기준 갱신({rec.get('size')}자)",
        ),
        "role_experience_saved": (KIND_EXPERIENCE, f"{rec.get('job')} 경험 적재"),
        "loop_circuit_breaker": (
            KIND_ALERT,
            f"수렴 경보 — 교차검증 {rec.get('cross')}회 미수렴",
        ),
        "intervention": (
            KIND_INTERVENTION,
            f"사용자 개입 — {_clip(rec.get('text'), 80)}",
        ),
        ...
        "continue_incomplete": (
            KIND_WORK,
            f"이어가기(seg {rec.get('seg')}, 진행={rec.get('progressed')})",
        ),
    }
    if ev in m:
        kind, summary = m[ev]
    ...
    return Event(
        ...
        kind=KIND_RAW,
        project=proj,
        summary=f"{ev} {_clip(extra, 60)}",
        payload={"event": ev, **extra},
    )
