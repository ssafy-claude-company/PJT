    ...
    if role == "leader":
        ...
        async def set_goal(args):
            ...
            if not getattr(flow, "gap_checked", False) and not _has_std and not _max_na:
                ...
                return _ok("확정 보류(최대화 기준 — 최대 표준 *기록* 강제, 재호출만으론 통과 안 됨): 이 시스템의 "
                           "전제는 '요청을 문자 그대로 최소로'가 아니라 '가용 외부자원으로 만들 수 있는 **최대** "
                           "품질'입니다(임계값 만족 아님 — 최대화). **이 작품과 같은 종류의 *실제 훌륭한 예*를 "
                           "WebSearch로 찾아**(상상 말 것 — LLM은 자기 산출을 기준 삼아 '평범=충분'으로 수렴하니 "
                           "실제 레퍼런스가 외부 기준), 그 *최대 버전*이 갖춘 품질·범주의 전체 표준을 **`standard` "
                           "인자에 적어 박으세요** — 단 *리더 혼자 정하지 말고* 각 도메인이 *자기 분야*의 실제 "
                           "훌륭한 예를 대조해 *자기 도메인 최대 기준*을 meet로 기여하게(standard는 그 합집합으로 "
                           "누적 — 품질 바를 한 명이 정하면 '품질이 1명 지능에 인질'). 마감 검증이 이 최대 대비 "
                           "*갭*으로 판정 — '돌아가나'가 아니라 '실제 최선만큼인가'. 정말 이 작품엔 최대화할 품질 "
                           "차원이 없으면(순수 내부 유틸 등) goal이나 standard에 **'[최대화 N/A: <사유>]'**를 적어 "
                           "재호출하세요(빈 재호출은 통과 안 됨 — 사유 필수).")
        ...
        async def complete_task(args):
            ...
            if has_product and not cc_ok and third:
                ...
                standard_v = ("\n[최대성 기준 — 실제 *최대*와 대조 (PHASE 3, 외부 grounding)] 이 작품의 *최대 표준*"
                              "입니다(목표 단계에서 실제 exemplar에 앵커해 박은 것 — 목적은 '돌아가나'가 아니라 '가용 "
                              "자원으로 만들 수 있는 *최대*만큼인가'). 검증자(다른 도메인)에게: 워크스페이스를 **직접 "
                              "ls/read로 *실측***해 이 표준 대비 **무엇이 못 미치나(below-max)·통째로 빠졌나**를 *아티팩트 "
            ...
            if _std_bind and not re.match(r"^\s*\[\s*최대화\s*(?:N\s*/?\s*A|면제|불필요)", _std_bind):
                ...
                _std_hdr = bool(re.search(r"\[\s*최대성\s*(?:검증|충족|회계|드롭)\s*\]", _res2))
                ...
                if not (_std_hdr or _std_na):
                    ...
                    return _ok("마감 보류(최대성 검증 — 최대 표준 대비 항목 회계 강제, 재호출만으론 통과 안 됨): 이 "
                               "Task엔 팀이 합의한 **최대 표준**(실제 exemplar 기준)이 박혀 있습니다 —\n"
                               + _speech_clip(_std_bind, 1200) + "\n\n각 항목이 산출물에 **실제로 그 최대만큼 "
                               "구현됐는지** result에 **'[최대성 검증]'** 헤더로 항목별 (충족 — 증거 / [드롭] <왜 이 "
                               "작품엔 과한지>)를 적어 재호출하세요 — '돌아간다'가 아니라 '이 최대 기준대로 됐나'가 "
                               "기준입니다(미달은 그 도메인 owner에게 개선 위임 후 재검증). 표준이 통째로 부적용이면 "
                               "**'[최대성 N/A: <사유>]'**(빈 재호출 통과 안 됨 — 사유 필수).")
