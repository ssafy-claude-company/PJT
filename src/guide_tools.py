    ...
    if role == "leader":
        ...
        async def complete_task(args):
            ...
            if has_product and flow.current.cross_checks == 0 and third:
                ...
                per_item = ("goal의 각 부분이 '존재하나'가 아니라 **사용자/플레이어로서 처음부터 끝까지 써본 "
                            "경험**으로 — **렌더된 화면을 스크린샷으로 찍어 Read로 직접 '눈으로 보고'**(DOM 존재가 "
                            "아니라 '실제로 이렇게 보인다'를 시각으로 확인 — 회색 도형인지 상용 제품처럼 보이는지), "
                            "**같은 종류의 훌륭한 예를 WebSearch로 실제로 찾아 대조**해 '재밌나·쓸 만한가? 뭐가 "
                            "싸구려 같고 답답한가? 뭐가 빠지거나 떨어지나? 상용 수준이면 당연히 있을 게 없나?'를")
