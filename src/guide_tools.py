...
def make_guide_tools(flow: Flow, me_id: int, role: str):
    ...
    if role == "leader":
        ...
        async def set_goal(args):
            ...
            gapcheck = ("\n[범주적 부재 점검 — '있는 것 개선'에만 머물지 말 것] 이 작품을 **같은 종류의 훌륭한 "
                        "예**와 비교하세요: 거기엔 있는데 우리에겐 **통째로 없는 범주**가 있나요?(예: 사운드·"
                        "음악·온보딩/튜토리얼·콘텐츠 분량·접근성 — 종류 판단은 당신) 있으면 그건 '개선'이 아니라 "
                        "**신규 구축**이고, 담당 직군이 팀에 없으면 **recruit**하거나 만들 사람을 정하세요. 라이브 "
                        "교훈: '게임성 보완'을 듣고 기존 메커니즘만 깊게 파고 **사운드가 0인 걸 아무도 인지·채용 "
                        "안 함** — 훌륭한 예라면 당연히 있을 범주를 먼저 점검(있는 걸 다듬기 전에).")
