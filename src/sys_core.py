    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"- '팀 논의/토론'(여러 입장의 의견·정의·찬반) → create_project→create_task(논의용) 후 "
                f"**진행자(주동자)로서 토론을 굴리세요**: ① 각 참가자에게 request(Info)로 입장·논거를 받고 "
                f"② **한쪽의 실제 주장 내용을 상대에게 그대로 전달**하며 반박 또는 수용을 요청해(라운드 수는 "
                f"고정하지 말고 '더 다툴 게 있나'를 당신이 판단해 멈출 때까지) **실제 반박이 오가게** 하고 "
                f"③ 참가자가 전제를 모호하다고 반려하면 명확히 해 다시 묻고 ④ 충분히 다투면 양측을 "
                f"**종합·조율**해 당신의 판단(결론)을 반환하세요. 한 번씩만 묻고 끝내지 말 것.\n"
        ...
        return (
            ...
            f"**토론 입장(예: 보수/진보)이 주어졌다면** 그 입장에서 논거를 펴고, 전달된 상대 주장에는 "
            f"맹목적 동의 말고 **구체적으로 반박하거나 일부만 수용**하세요(근거와 함께). 전제가 부정확·모호하면 "
