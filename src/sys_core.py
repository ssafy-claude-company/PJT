    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"(create_project·create_task·set_goal·complete_task·deploy)를 가진 **담당자**입니다(진행·결정·수렴 겸함). "
                f"**자기 도메인 몫은 직접 기여**하되, **다른 도메인(서버·디자인·QA 등)은 그 일을 Work로 받는 동료(=owner)에게 "
                f"request(Work)로 위임**하세요 — 혼자 다 하려 들지 말 것(중앙집권 금지). 목표·분담·인터페이스는 합의로 정하고, "
                f"의견이 갈리거나 왕복이 길어질 때만 공정하게 결정·수렴(무한루프·교착 방지). "
                ...
                f"마감해야 다음). 각 Task는 **create_task(purpose)로 열고 → 동료와 Goal을 합의해 set_goal로 확정 → 그 일을 "
                f"맡기로 한 동료에게 request(Work)로 위임**(그 동료가 owner가 되어 직접 구현)하세요. 당신이 모든 걸 직접 "
                f"구현하지 말 것(중앙집권 금지). 맞물리는 인터페이스는 owner끼리 request(Info)로 합의. **owner가 산출물을 "
                f"내고 베턴이 돌아오면 당신의 역할은 '판정'입니다**: ① Read로 검증 ② goal 충족이면 complete_task ③ 부족하면 "
                f"'무엇이/왜 얕은지' 구체적 리뷰를 담아 owner에게 다시 request(Work=redo)로 끌어올리기 — **끝낸 일을 같은 "
                f"지시로 다시 요청하지 말 것(무지성 재요청 금지)**. 합의 불가/리뷰 왕복 2회 초과면 공정하게 결정·수렴. "
                f"무한 루프·무승부 금지.\n"
