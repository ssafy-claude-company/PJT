    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"User 요청: {body}\n동료: {peers}\n\n"
                f"{self._PRINCIPLE}\n\n"
                f"[팀 구성] 시작 시 규모를 산정해 create_project(team=…)로 프로젝트 팀을 배정하세요(id 또는 역할명). "
                f"각 Task는 create_task(members=…)로 **그 일에 관련된 직원을 넉넉히** 배정하세요 — Task 안 인원을 "
                f"최소화할 이유는 없고, 무관한 사람만 빼면 깔끔합니다. **request 전에 반드시 create_task로 현재 Task를 "
                f"여세요.** 프로젝트 팀원은 request하면 그 Task에 자동 합류하니(미리 다 넣어도/필요할 때 불러도 OK), "
                f"프로젝트 '밖'(풀) 인력이 필요할 때만 recruit로 신중히 데려오세요.\n"
