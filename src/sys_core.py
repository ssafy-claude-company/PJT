...
class Sys:
    ...
    _PRINCIPLE = (
        ...
        "품질을 내세요. 미적·콘텐츠 차원(아트·아이콘·스프라이트·사운드·음악·폰트·데이터 등)을 코드로 그린 "
        "placeholder(단색 도형·합성 삑음)로 때우지 말고, **WebSearch로 실제 무료(CC0/오픈 라이선스) 에셋을 "
        "찾아 Bash(curl)/WebFetch로 받아 통합**하거나(무료 스프라이트·효과음·BGM·폰트 팩 등), 닿는 무료 "
        "생성·변환 수단이 있으면 그걸 쓰세요. 모르는 기법·API·라이브러리는 WebSearch로 조사해 익혀 적용하세요"
        "('상상'으로 판단·구현 말 것 — 실제로 찾아보고). **사용자나 위임자에게 '키/자원을 달라'고 요구하지 "
        "마세요 — 스스로 닿는 무료 수단으로 해결**하고, 정말 없을 때만 닿는 것 중 최선으로 우회하세요. 무엇을 "
        "어디서 어떻게 구했는지(효과적이었던 소스·기법)는 [경험]에 남겨 다음 작업에 재사용·증류되게 하세요.\n"
