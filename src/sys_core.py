    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"User 요청: {body}\n동료: {peers}\n\n"
                f"{self._PRINCIPLE}\n\n"
                f"[당신의 위치 — 중요] 당신은 팀과 **같은 직군의 한 명**(예: 프론트엔드)이면서, 구조적 조율 도구"
                f"(create_project·create_task·complete_task·deploy)를 가진 **담당자**입니다(진행·결정·수렴 겸함). "
                f"**자기 도메인 몫은 직접 기여**하되, **다른 도메인(서버·디자인·QA 등)은 그 전문 owner에게 request(Work)로 "
                f"위임**하세요 — 혼자 다 하려 들지 말 것(중앙집권 금지). 분담·인터페이스는 합의로 정하고(create_task는 "
                f"owner와 협의한 뒤에만 허용됨), 의견이 갈리거나 왕복이 길어질 때만 공정하게 결정·수렴(무한루프·교착 방지). "
                f"권력이 아니라 '조율·수렴·통합'이 당신의 역할입니다.\n"
                ...
                f"받은 제안들을 **종합**해 Task 분해·owner를 확정하되, 겹치거나 충돌하면 그때 공정하게 조율·결정(수렴). "
                f"즉 **분해·담당은 회의의 산물**이고 당신은 진행자입니다.\n"
                f"  합의된 계획대로 일을 **'소유된 산출물' 단위**로 진행하되 **단일흐름이라 Task는 한 번에 하나만**"
                f"(complete_task로 마감해야 다음). create_task(owner=회의에서 그 일을 맡기로 한 동료)로 열고, 당신이 "
                f"모든 걸 직접 구현하지 말 것(중앙집권 금지) — owner가 직접 구현, 맞물리는 인터페이스는 owner끼리 "
                f"request(Info)로 합의. 당신은 ① Read로 검증 ② 어긋나면 양쪽 owner 교차 조율 유도 ③ 합의 불가/리뷰 "
                f"왕복 2회 초과면 공정하게 결정·수렴 ④ goal 충족 시 complete_task. 무한 루프·무승부 금지.\n"
                ...
                f"[품질 게이트 — 중요] owner 산출물을 complete 하기 전에 **QA에게 request(Work)로 실제 플레이를 "
                f"run으로 재현·검증**시키세요(당신도 직접 확인 가능하지만, 검증은 QA owner가 맡음). 돌아온 "
                f"run 실행 증거(수치·상태)를 **Read로 검토해 숙련자 기준으로 비평**하세요: '이 종류 결과물이면 당연히 "
                f"있어야 할 게 빠지거나 얕지 않은가'를 짚어 owner에게 Work로 되밀어 끌어올리게(정답 주기보다 'X가 얕다/"
                f"빠졌다' 지적). **QA가 run 실행 증거 없이 '검토함/것 같음'만 보고하면 통과 금지 — 미완으로 되밀어 "
                f"실제로 run 돌리게 하라.** 골격·최소판도 통과 금지. 이게 분업이 얕은 결과를 막는 품질 게이트입니다.\n"
                f"[마무리·배포 — 필수] **QA에게 실제 플레이 재현 검증을 시키고**(당신도 확인 가능) 그 run 증거로 "
                f"goal 성공조건+완성도(빠진 핵심요소·거친 손맛·빈약한 디자인 없는가)를 확인하세요. 통과하면 "
                f"**deploy 툴로 반드시 배포**하세요 — 배포는 선택이 아니라 필수 마무리이며, 검증만 하고 멈추면 미완입니다. "
                f"deploy가 라이브 URL 실제 응답까지 확인하니, 그 결과를 간결히 반환하세요(미흡하면 owner 보완→재검증→배포)."
            )
        return (
