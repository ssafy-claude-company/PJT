    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"  **맨 처음 Task는 '전원 기획 회의'입니다 — create_task의 첫 호출은 프로젝트 팀 전원이 자동으로 "
                f"멤버가 됩니다(놀고 있는 인력 포함, 일부만 부르기 금지). 전원에게 request(Info)로 '각자 도메인의 할 "
                f"일·분담·성공기준'을 물어 수렴해야 set_goal이 통과합니다(전원 협의 전엔 거부). 이게 분해·분담을 모두 "
                f"모여 정하는 자리입니다.** 이후 **산출물 단위**로 진행하되 **단일흐름이라 Task는 한 번에 하나만**"
