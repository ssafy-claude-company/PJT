    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"[품질 게이트 — 중요] owner 산출물을 complete 하기 전에 **당신이 사용자처럼 직접 경험**(run으로 "
                f"구동·조작)하고 **숙련자 기준으로 비평**하세요: '이 종류 결과물이면 당연히 있어야 할 게 빠지거나 "
                f"얕지 않은가'를 구체적으로 짚어 owner에게 Work로 되밀어 깊이를 끌어올리게 하세요(정답을 미리 주기보다 "
                f"'X가 얕다/빠졌다'를 지적). 골격·최소판이면 통과 금지 — 완성·정돈됐을 때만 complete. **특히 QA·검증 "
                f"산출물은 run 실행 증거(실제 시뮬레이션의 수치·상태)가 없으면 통과 금지 — '검토함/것 같음/예상됨'은 "
                f"미완으로 되밀어 run으로 직접 재현하게 하라.** 이게 분업이 얕은 결과를 내는 걸 막는 품질 게이트입니다.\n"
                f"[마무리 검증] 보고 전에 run 툴로 **실제 사용 시나리오를 재현**해 goal의 성공 조건 + **완성도"
                f"(빠진 핵심 요소·거친 손맛·빈약한 디자인이 없는가)**까지 충족되는지 확인하세요(구동 여부가 아니라 "
                f"의도한 결과·경험의 질 — 미흡하면 미완으로 보고 owner에게 보완 요청). 확인 뒤 결과를 간결히 반환하세요."
            )
        return (
