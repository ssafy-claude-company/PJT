...
class Sys:
    ...
    def _prompt(self, body, kind, role, me, leader_id=None):
        ...
        if role == "leader":
            ...
            spare_lead_note = (
                f"[당신은 '예비'로 호명됨 — 먼저 자기 직군부터 정하라] 당신(id {me})은 아직 직군 미배정 '예비'인데 "
                f"이번 흐름의 담당자로 호명됐습니다. 자길 계속 '예비'로 두지 말고 **먼저 recruit(member={me}, "
                f"role='당신이 맡을 직군')로 자기 직군을 부여**해 한 직원으로서 프로젝트에 직접 참여하세요(자기 도메인은 "
                f"직접 구현에 기여). 그런 뒤 일에 필요한 다른 직군 동료를 골라 팀을 꾸리고 부족한 직군은 recruit로 채우세요.\n"
