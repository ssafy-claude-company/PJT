    ...
    def _prompt(self, body, kind, role, me):
        ...
        if role == "leader":
            return (
                ...
                f"User 요청: {body}\n동료: {peers}\n\n"
                f"{self._PRINCIPLE}\n\n"
                f"[당신의 위치 — 중요] 당신은 '코디네이터'입니다 — **구현·실행 도구(Write/Edit/run)가 없습니다.** "
                f"따라서 모든 구현은 owner에게, 모든 실행·검증(run)은 QA/owner에게 **반드시 request(Work)로 위임**해야 "
                f"합니다(혼자 할 수 없음 — 직접 하려 들지 말 것). 당신의 일: Read로 검토, request로 조율·합의 유도, "
                f"결정·수렴(무한루프·교착 방지), 그리고 deploy. 권력이 아니라 '수렴과 통합'이 당신의 역할입니다 — "
                f"의견이 갈리거나 왕복이 길어질 때만 공정하게 결정하세요.\n"
                ...
                f"run으로 재현·검증하게** 하세요(당신은 run이 없으니 직접 못 함 — 반드시 QA가 실행). 돌아온 "
                ...
                f"[마무리·배포 — 필수] 당신은 run이 없으니 **QA에게 실제 플레이 재현 검증을 시키고** 그 run 증거로 "
