...
def test_set_goal_점유도메인은_면제아니라_1회대기보류_후_의식적진행():
    ...
    from organt_core.rule.communication import Engagement
    g = FakeGuide()
...
def test_정밀복구_깊은체인_가장깊은워커_재개():
    ...
    *가장 깊은 활성 워커(디자이너)*를 그 원문으로 재개 owner로 세운다 — 깊은 작업이 리더로 안 튄다. 전체 체인
    ...
    import tempfile
    from organt_core.rule.communication import Kind as CK
    g = FakeGuide()
...
def test_전역점유_타흐름_동료는_Kind불문_차단_응답시_즉시해제():
    ...
    import pytest
    from organt_core.rule.communication import BusyInOtherFlow, CommunicationManager, Engagement
    eng = Engagement()
...
def test_전역점유_상신_강제정리도_해제대칭():
    ...
    from organt_core.rule.communication import CommunicationManager, Engagement
    eng = Engagement()
    a = CommunicationManager(0)
    a.attach_engagement(eng, "P-A")
