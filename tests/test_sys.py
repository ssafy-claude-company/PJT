...
def test_팀기여의무_부른직군이_실작업0이면_1회보류_RFC009():
    """[팀 기여 의무 — RFC-009] 교차 검증(cross_checks)과 **독립**. 팀에 부른 직군이 회의 발언만 하고
    실작업·검증 0(act_by==0: Write/Edit/run 한 번도 없음)이면, 검증이 끝났어도(cross_checks>0) 완료를
    1회 보류해 '이 도메인이 작품에 반영 안 됨'을 환기한다 — 라이브 P-010: VFX·디자이너·사운드 등
    폴리시 직군이 실구현 0인 채 마감돼 "단순 나열 웹·타격감 없는 게임"이 됨(발언≠기여). ① Work 위임
    ...
    f.current.cross_checks = 1             # 교차 검증은 통과 상태
    # 14(VFX)는 act_by==0: 회의 발언만 함 → 폴리시가 작품에 반영 안 됨
    r1 = asyncio.run(t["complete_task"].handler({"result": "끝"}))
    txt1 = r1["content"][0]["text"]
    assert "완료 보류(팀 기여 의무" in txt1 and f.current is not None      # 1회 보류
    assert "VFX" in txt1                                                 # 잠수 직군 지목
    assert "request(Work)" in txt1 and "팀에서 빼" in txt1 and "재호출" in txt1  # 3선택지
    assert f.current.contrib_checked is True                            # 보류 1회 마킹
    r2 = asyncio.run(t["complete_task"].handler({"result": "끝"}))       # 재호출
