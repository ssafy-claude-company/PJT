...
from organt_core.guide_tools import (Flow, make_guide_tools, _wants_real_data,
...
def test_서브프로세스_사망_143은_일시오류로_재시도대상():
    ...
    from organt_core.organt import _is_transient_api_error
...
def test_run_셸은_배포비밀을_못_읽는다(tmp_path, monkeypatch):
    ...
    import os as _os
    from organt_core.guide_tools import _scrubbed_run_env, _is_secret_env
...
def test_일로직업획득_채용은잠정_첫실작업에_영속승격():
    ...
    from organt_core.permissions import make_pre_tool_use_hook, organt_allowed_tools
    g = FakeGuide()
...
def test_워커_턴_타임아웃은_인프라실패로(monkeypatch):
    ...
    monkeypatch.setattr("organt_core.sys_core.build_guide_server", lambda *a, **k: object())
...
def test_봇은_이_채널의_상황을_보고_답한다():
    ...
    import asyncio
    from organt_core.protocol import Request, Response
...
def test_set_goal_점유도메인은_면제아니라_1회대기보류_후_의식적진행():
    ...
    from organt_core.communication import Engagement
    g = FakeGuide()
...
def test_정밀복구_깊은체인_가장깊은워커_재개():
    ...
    *가장 깊은 활성 워커(디자이너)*를 그 원문으로 재개 owner로 세운다 — 깊은 작업이 리더로 안 튄다. 전체 체인
    ...
    import tempfile
    from organt_core.communication import Kind as CK
    g = FakeGuide()
...
def test_하트비트_일하는워커는_침묵타임아웃_안걸림():
    ...
    s = Sys(g, guild_id=1, organt_builder=lambda oid, srv, role, flow=None: _Worker(flow),
            bot_info={11: "L", 12: "M"})
    s.turn_timeout = 0.5
    import organt_core.sys_core as sc
...
def test_강제배포는_완료Task가_있을때만(tmp_path, monkeypatch):
    ...
    for k, v in (("GH_PAT", "x"), ("GH_USER", "u"), ("RENDER_KEY", "k"),
        ...
        monkeypatch.setenv(k, v)
    deployed = {"n": 0}
    monkeypatch.setattr("organt_core.deploy.deploy_sync",
...
def test_create_project는_id기반_작업공간과_배포슬롯(tmp_path):
    ...
    import os as _os
    from organt_core.guide_tools import deploy_service_name
    base = str(tmp_path)
    g = FakeGuide()
...
def test_발언_안전망은_침묵절단하지_않는다():
    ...
    from organt_core.guide_tools import _speech_clip
...
def test_워커도구에_WebSearch_포함_RFC011():
    ...
    from organt_core.builder import WORKER_BASE_TOOLS   # [계층 분리] Core 빌더로 이동(종전 src.main)
...
def test_전역점유_타흐름_동료는_Kind불문_차단_응답시_즉시해제():
    ...
    import pytest
    from organt_core.communication import BusyInOtherFlow, CommunicationManager, Engagement
    eng = Engagement()
...
def test_전역점유_상신_강제정리도_해제대칭():
    ...
    from organt_core.communication import CommunicationManager, Engagement
    eng = Engagement()
    a = CommunicationManager(0)
    a.attach_engagement(eng, "P-A")
...
def test_배포검증_라이브가_산출물과_다르면_성공선언_불가(tmp_path):
    ...
    from organt_core.deploy import _verify_live_assets
    pub = tmp_path / "public"
    pub.mkdir()
...
def test_이어가기_본문에_팀·소유_시스템사실_재주입(tmp_path):
    ...
    async def fake_run_turn(flow, oid, body, kind, role):
        ...
        if calls["n"] == 1:                          # 1세그먼트: Task를 연 채 끝남 → 이어가기 유발
            from organt_core.guide_tools import TaskRef
            from organt_core.protocol import TaskStatus
...
def test_배포_변경없는_재배포_차단_anti_thrash(monkeypatch):
    ...
    import organt_core.deploy as dp
    import os
...
def test_배포_런어웨이_횟수상한_차단(monkeypatch):
    ...
    import organt_core.deploy as dp, os
...
def test_배포_진행중_재호출은_대기_새배포_트리거_금지():
    ...
    import organt_core.guide_tools as gt
    f = Flow(FakeGuide(), channel_id=500, guild_id=1, leader_id=11, bot_info={11: "L"})
    f.start_root("root")
    f.workspace = "/tmp/ws-x"
...
def test_배포풀_자가정리_고아만_오래된순_삭제_참조링크_보존(monkeypatch):
    ...
    from organt_core import deploy
    svcs = []
