...
8. **전직 정책(사용자 확정)**: 원칙은 1봇 1직업 1기억(전문화). 직군 보유 봇의 직군 변경은
   **예외 둘 중 하나일 때만** — ① 풀에 예비가 0명 ② 새 직군이 기존과 '비슷한 일'(토큰 공유).
   허용 시 겸직이 아니라 **교체**(라벨·jobs.json·Discord 역할 모두 새 직군 하나). 그 외엔 거부
   ("예비를 그 직군으로 뽑으라"). recruit 본 채용·Task 전 자기직군 두 경로 모두 적용.
