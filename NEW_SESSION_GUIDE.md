...
- 감시는 **감독자 1개**(flock이 구조 보장) — 유휴 시간대의 세션 Monitor류 상시 감시는 금지(토큰 누수).
  **하트비트(주기 깨움)는 기본 해제**(사용자 확정 2026-06-12: '계속 살아있는 상태는 토큰
  소비가 너무 크다 — 인간 방식에서 장점만 증류하라'). 항상 깨어 있음은 인간 뇌의 장점이
  아니라 생물학적 제약이고, 기계의 고유 장점은 **무손실 동결·재개**다 — 동면이 와도 졸업
  라우팅·open_task·시초 계기판이 피해 0으로 잇는다(라이브 검증 다수). 비용 구조: 세션 깨움
  1회(거대 컨텍스트 재생)가 워커 턴 재실행보다 비싸다. 흐름이 멈춰 보이면 아무 메시지로
  깨우면 그 자리에서 재개된다. **사용자가 '이번 작업은 지켜봐 달라'고 명시한 경우에만**
  짧은 타임아웃 Monitor를 무장(목적은 박스 밖 타임아웃 알림 = 유일한 외부 깨움).
  근본 해법은 상시 호스트(로컬 PC + 구독 OAuth — 동면·깨움 비용 자체가 소멸).
