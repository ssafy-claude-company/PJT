...
- 감시는 **감독자 1개**(flock이 구조 보장) — 유휴 시간대의 세션 Monitor류 상시 감시는 금지(토큰 누수).
  **예외 1개: '작업 중 하트비트'** — 흐름이 도는 동안만 타임아웃 **~8분** Monitor를 1개 무장한다
  (관측된 동면 임계가 유휴 3~8분이라 15분 간격은 '작업 중 동면 창'을 남겼다 — 품질>토큰).
  목적은 감시가 아니라 **깨우기**: 컨테이너 동면은 박스 안 코드(감독자 포함)로는 못 막고,
  **박스 밖(하니스)에서 오는 타임아웃 알림만이** 잠든 박스를 되살린다. 라이브 실증: 흐름
  30~60분 > 세션 유휴 한도라 '작업 도중 동면'이 반복됐고, 시끄러운 모니터 정리(항목 25·28)가
  토큰 누수와 함께 **우연한 생존 유지까지 제거**했던 것(사용자 통찰). 깨어나면: 흐름이 아직
  돌면 점검 후 재무장, 유휴면 무장하지 않는다(잠들어도 됨 — 부팅 복구·졸업 라우팅이 피해 0).
  비용: 작업 중에만 시간당 ~7회 짧은 깨움(품질>토큰 확정에 따른 수용).
